import "MasterCard.sol";

/**
 * Contract to manage and store master nfc cards (UID's) and their contracts 
 */
contract MasterCardManager {

    event NewMasterCard(address contractAddress, bytes32 id, bytes32 uid);
   ...
   function value(bytes32 key) constant returns (address addr) {
    ...
    /**
     * Adds a new master card with the specified attributes
     */
    function addMasterCard(bytes32 _id, bytes32 _uid) returns (bool) {
        MasterCard masterCard = new MasterCard(_id, _uid);
        insert(_id, masterCard);
        NewMasterCard (masterCard, masterCard.id(), masterCard.uid());
        return true;
    }
